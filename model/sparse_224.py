...
class Sparse(object):
    ...
    def build_graph(self, is_training = True):
        ...
        with tf.variable_scope('sparse'):
            ...
            x_additional=tflearn.layers.core.fully_connected(x,2048,activation='linear',weight_decay=1e-4,regularizer='L2')
            ...
            #16 128
            x=tflearn.layers.conv.conv_2d_transpose(x,256,[5,5],[14,14],strides=2,activation='linear',weight_decay=1e-5,regularizer='L2')
            x2=tflearn.layers.conv.conv_2d(x2,256,(3,3),strides=1,activation='linear',weight_decay=1e-5,regularizer='L2')
            x=tf.nn.relu(tf.add(x,x2))
            x=tflearn.layers.conv.conv_2d(x,256,(3,3),strides=1,activation='relu',weight_decay=1e-5,regularizer='L2')
            
            #32 64
            x=tflearn.layers.conv.conv_2d_transpose(x,128,[5,5],[28,28],strides=2,activation='linear',weight_decay=1e-5,regularizer='L2')
            x1=tflearn.layers.conv.conv_2d(x1,128,(3,3),strides=1,activation='linear',weight_decay=1e-5,regularizer='L2')
            ...
            x = tf.reshape(x,(tf.shape(x)[0],784,3))

            x_additional=tflearn.layers.core.fully_connected(x_additional,1024,activation='relu',weight_decay=1e-3,regularizer='L2')
            x_additional=tflearn.layers.core.fully_connected(x_additional,240*3,activation='linear',weight_decay=1e-3,regularizer='L2')
            x_additional=tf.reshape(x_additional,(tf.shape(x)[0],240,3))
