...
from layer.gat_layers import attn_feature
...
class Deform(object):
    ...
    def build(self):
        if self.mode == 'train':
            self.build_graph(bn_decay = 0.99)
    ...
    def build_graph(self, is_training = True, bn_decay = None):
        ...
        adj_matrix = tf_util.pairwise_distance(pc_feats)
        nn_idx = tf_util.knn(adj_matrix, k=k)
        n_heads = 4
        attns = []
        local_features = []
        for i in range(n_heads):
            edge_feature, coefs, locals = attn_feature(pc_feats, 16, nn_idx, activation=tf.nn.elu, in_dropout=0.6,
        ...
        neighbors_features = tf.concat([tf.expand_dims(pc_feats, -2), neighbors_features], axis=-1)

        pc_feats = tf_util.conv2d(neighbors_features, 64, [1, 1], padding='VALID', stride=[1, 1],
                                bn=True, is_training=is_training, scope='gapnet1', bn_decay=bn_decay)
        pc_feats = tf.reduce_max(pc_feats, axis=-2, keep_dims=False)
        local_max = tf.reduce_max(tf.concat(local_features, axis=-1), axis=-2, keep_dims=False)
        ...
        point_feats = tf.concat([local_max,adain1,adain2,adain3,adain4,adain5,project],-1)
