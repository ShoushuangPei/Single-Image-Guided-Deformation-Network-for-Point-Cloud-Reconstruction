...
class Deform(object):
    ...
    def build_graph(self, is_training = True):
        ...
        k=20
        bn_decay=0.99
        local = tf.layers.dense(point_feats,512,activation=None,use_bias=True)

        adj_matrix = pairwise_distance(local)
        nn_idx = knn(adj_matrix, k=k)
        edge_feature = get_edge_feature(local, nn_idx=nn_idx, k=k)
        net = conv2d(edge_feature, 256, [1,1],
        ...
        local_feature = conv2d(tf.concat([net1, net2], axis=-1), 512, [1, 1], 
                        padding='VALID', stride=[1,1],
                        bn=True, is_training=is_training,
                        scope='agg', bn_decay=bn_decay)
        local_feature = tf.reduce_max(local_feature, axis=-2, keep_dims=False)

        point_feats = tf.concat([point_feats, local_feature],-1)
