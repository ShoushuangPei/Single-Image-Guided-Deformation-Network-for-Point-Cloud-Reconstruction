...
class Deform(object):
    ...
    def build_graph(self, is_training = True):
        ...
        global_feats = encoder_with_convs_and_symmetry(in_signal=self.init_pc, n_filters=[32,64,64],filter_sizes=[1],strides=[1],b_norm=True,verbose=True)
        global_feats = tf.tile(tf.expand_dims(global_feats, axis=1), [1, 256, 1]) 
        _, local_feats, _ = pointnet_sa_module(self.init_pc, None, npoint=256, radius=0.1, nsample=8, mlp=[32,32,64], mlp2=None, group_all=False, is_training=False, bn_decay=None, scope='local_feat', bn=False)
        radius_list = [round(0.1*scale,2) for scale in [1.,1.1,1.2]]
        print(radius_list)
        _, local_msg_feats = pointnet_sa_module_msg(self.init_pc, None, npoint=256, radius_list=radius_list, nsample_list=[8,16,32], mlp_list=[[8,16], [16,32], [16,32]], is_training=False, bn_decay=None, scope='mul_local_feat',  bn=False)
        ...
        point = graphX(concat_feats, 512, num_instance=512, rank=128, bias=True, activation=tf.nn.relu)
