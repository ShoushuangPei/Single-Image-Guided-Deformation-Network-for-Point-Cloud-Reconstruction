...
class Deform(object):
    ...
    def build_graph(self, is_training = True):
        ...
        # point decoder
        point = graphX(point_feats, 512, num_instance=500, rank=125, bias=True, activation=tf.nn.relu, is_training=is_training)
        point = graphX(point, 256, num_instance=1000, rank=250, bias=True, activation=tf.nn.relu, is_training=is_training)
        point = graphX(point, 128, num_instance=2000, rank=1000, bias=True, activation=tf.nn.relu, is_training=is_training)
...
def graphX(pc_feats, out_channel, num_instance, rank, bias, activation, is_training):
    #main
    main = tf.layers.dense(pc_feats,out_channel,activation=tf.nn.relu,use_bias=True)
    main = dgcnn(main, out_channel,is_training=is_training)
...
def dgcnn(pc_feats, out_channel, is_training, k=20, bn_decay=0.99):
    adj_matrix = pairwise_distance(pc_feats)
    nn_idx = knn(adj_matrix, k=k)
    edge_feature = get_edge_feature(pc_feats, nn_idx=nn_idx, k=k)
    net = conv2d(edge_feature, out_channel / 2, [1,1],
                padding='VALID', stride=[1,1],
                bn=True, is_training=is_training,
                scope='dgcnn1_{}'.format(out_channel), bn_decay=bn_decay)
    ...
    net = conv2d(edge_feature, out_channel / 2, [1,1],
                padding='VALID', stride=[1,1],
                bn=True, is_training=is_training,
                scope='dgcnn2_{}'.format(out_channel), bn_decay=bn_decay)
    net = tf.reduce_max(net, axis=-2, keep_dims=True)
    net2 = net

    local_feature = conv2d(tf.concat([net1, net2], axis=-1), out_channel, [1, 1], 
                        padding='VALID', stride=[1,1],
                        bn=True, is_training=is_training,
                        scope='agg_{}'.format(out_channel), bn_decay=bn_decay)
