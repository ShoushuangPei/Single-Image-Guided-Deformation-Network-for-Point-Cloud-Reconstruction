...
def dump_image_point():
    ...
    point_input_folder = 'pointcloud_3dr2n2'
    image_output_folder = 'image'
    point_output_folder = 'point'
    image_number = 24
    ...
    for cat in cats:
        ...
        train_point_models.extend([os.path.join(point_prefix, point_input_folder, model) for model in train_models_dict[cat]])
        ...
        train_img_shape = (len(train_img_path), 224, 224, 3)
        ...
        for i in trange(len(train_img_path)):
            image_array, point_array = load_data(train_img_path[i], train_pt_path[i])
        ...
        test_point_models.extend([os.path.join(point_prefix, point_input_folder, model) for model in test_models_dict[cat]])
        ...
        test_img_shape = (len(test_img_path), 224, 224, 3)
        ...
        for i in trange(len(test_img_path)):
            image_array, point_array = load_data(test_img_path[i], test_pt_path[i])
