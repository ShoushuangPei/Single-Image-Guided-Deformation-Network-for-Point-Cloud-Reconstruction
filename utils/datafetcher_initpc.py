...
def get_batch_init_pc(batch_size, num_points):
    ...
    # for i in batch_size:
    #     batch += init_pointcloud_loader(num_points)

class DataFetcher(threading.Thread):
    def __init__(self, mode, batch_size = 32, epoch = 10, num_points = 1024):
                    ...
                    with h5py.File(os.path.join(self.image_path, '{}.h5'.format(cat)), 'r') as fi:
                        with h5py.File(os.path.join(self.point_path, '{}.h5'.format(cat)), 'r') as fp:
                            for i in range(0, batch * self.batch_size, self.batch_size):
                                if self.stopped:
                                    break
                                self.queue.put((fi['image'][i:i+self.batch_size].astype('float32') / 255.0, fp['point'][i:i+self.batch_size], get_batch_init_pc(self.batch_size, self.num_points)))
